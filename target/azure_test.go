...
func azureTarget() cloudauth.Target {
...
func TestAzureExchangeSuccess(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !strings.Contains(r.URL.Path, "tenant-123") {
	...
	creds, err := e.Exchange(context.Background(), oidcToken(), azureTarget())
	if err != nil {
...
func TestAzureRejectsSigV4WithBridgeGuidance(t *testing.T) {
	...
	e := NewAzureExchanger()
	sigv4 := &cloudauth.SourceToken{Kind: cloudauth.AWSSigV4, Value: `{"url":"x"}`}
	_, err := e.Exchange(context.Background(), sigv4, azureTarget())
	if !errors.Is(err, cloudauth.ErrNoFirstClassPath) {
