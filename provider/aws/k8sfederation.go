...
// A Kubernetes ServiceAccount federated to AWS *is* an OIDC role trust.

// k8sSubjectFormat is the sub claim a Kubernetes projected ServiceAccount token carries.
...
func k8sToRoleTrustSpec(spec *core.K8sServiceAccountFederationSpec) (*core.AWSRoleTrustOIDCSpec, error) {
	if spec.TargetCloud != core.AWS {
		// Unreachable through the manager, which routes on TargetProvider(), but this function is exported to the package and should not silently build an AWS role for a spec that asked for another cloud.
	...
	if strings.ContainsAny(spec.Namespace+spec.ServiceAccountName, "*?") {
		// A wildcard here would widen the trust to every ServiceAccount matching it.
		return nil, core.ErrValidation(
	...
	return &core.AWSRoleTrustOIDCSpec{
		...
		// Exact, not StringLike: the subject is fully known, so there is nothing for a pattern operator to do except accept more than was asked for.
...
// k8sRoleDescription records where the trust came from, so the role is attributable in the console without consulting cloud-auth's state file.
