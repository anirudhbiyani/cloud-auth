...
// Implements core's validation source interfaces so its trust-policy and permission checks run against live GCP state.
var (
	_ core.TrustPolicySource   = (*Provider)(nil)
	_ core.GrantedPolicySource = (*Provider)(nil)
)

// celLiteral extracts double-quoted string literals from a CEL attribute condition.
var celLiteral = regexp.MustCompile(`"([^"]*)"`)

func subjectsFromAttributeCondition(cond string) []string {
	if strings.TrimSpace(cond) == "" {
		// No attribute condition means the provider admits EVERY identity the issuer can mint — the classic confused-deputy hole, and Google's own docs call it out.
		return []string{"*"}
	}
	matches := celLiteral.FindAllStringSubmatch(cond, -1)
	if len(matches) == 0 {
		// A condition we cannot decompose (e.g. purely arithmetic).
		return []string{cond}
	}
...
func (p *Provider) TrustPolicy(ctx context.Context, ref core.MechanismRef) (*core.TrustPolicy, error) {
	...
	case wp.OIDC != nil:
		...
		if len(tp.Audiences) == 0 {
			// An unset allowed-audiences list does NOT mean "none accepted": GCP falls back to the provider's own resource name (in both its "//iam.googleapis.com/..." and https forms).
			full := wp.Name
			if full == "" {
				full = providerName
			}
...
// firstCELLiteral returns the first quoted literal in a CEL attribute condition, i.e. the concrete subject an operator scoped the provider to.
