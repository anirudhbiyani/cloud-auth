...
// Implements core's validation source interfaces so its trust-policy and permission checks run against live Entra state.
var (
...
func (p *Provider) TrustPolicy(ctx context.Context, ref core.MechanismRef) (*core.TrustPolicy, error) {
	// The client needed depends on where the credential lives: an app registration's FICs come from Graph, a user-assigned managed identity's from ARM.
	var (
	...
	if cred.Subject != "" {
		...
		// An empty subject would admit anything the issuer mints; surface it as the wildcard so the core validator fails it as unscoped.
		tp.Subjects = []string{"*"}
	}
	return tp, nil
}
...
func (p *Provider) GrantedPolicies(ctx context.Context, ref core.MechanismRef) ([]string, error) {
	...
	// Assignments are listed at subscription scope, which covers the scopes a setup would have granted at or below it.
...
// recordExpectedTrust stores the intended issuer/subject/audience on the mechanism ref so a later Validate can compare the live federated credential against what was configured.
