...
// versionTag must NOT wrap.
func versionTag(n int) string { return `"v` + strconv.Itoa(n) + `"` }

// The acceptance criterion: the remote backend passes the same suite as the file store.
...
// A losing compare-and-swap must re-read and retry, not overwrite.
...
// A retry budget has to be bounded, and running out has to say so rather than silently doing nothing.
...
// A document that is not JSON must fail loudly.
...
// A newer schema must not be rewritten by an older build, which would drop whatever fields it does not understand.
...
// An absent object is an empty store, not an error: the first operator to run setup against a fresh bucket must not have to create the object by hand.
