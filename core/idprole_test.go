...
// AWS shipped sts:RoleAuthorizedByIdp in July 2026: an identity provider embeds "https://aws.amazon.com/roles" in the token naming which roles it may assume, and STS enforces that as an allow-list BEFORE the trust policy is evaluated.
...
func TestIdPAuthorizedRole(t *testing.T) {
	...
	t.Run("token carries no claim at all", func(t *testing.T) {
		...
		// The message has to explain the evaluation order, or an operator will go looking at the subject condition — which is fine, and irrelevant.
	...
	// The condition is off by default and most policies will never carry it; firing on every one of them would be noise.
	...
	// Without knowing which role is being attempted there is nothing to compare against, and guessing would produce a finding nobody can act on.
...
// A null roles claim must reach this detector as ABSENCE, not as one authorized role that happens to be the empty string.
func TestNullRolesClaimReadsAsAbsent(t *testing.T) {
	...
	// The shape the bug produced, asserted so the difference is visible: one empty-string role takes the other branch entirely.
