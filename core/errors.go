...
func CategoryOf(err error) ErrorCategory {
	var caErr *CloudAuthError
	if errors.As(err, &caErr) {
		return caErr.Category
	}
	switch {
	case err == nil:
		return ""
	case errors.Is(err, ErrTrustMissing):
		return ErrCategoryAuth
	case errors.Is(err, ErrNotThisRuntime), errors.Is(err, ErrNonFederatableSource),
		errors.Is(err, ErrNoFirstClassPath), errors.Is(err, ErrRuntimeMismatch):
