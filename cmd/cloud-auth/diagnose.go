...
func diagnoseMintError(p preflight) []diagnosis {
	switch {
	case errors.Is(p.mintErr, cloudauth.ErrNonFederatableSource):
	...
	case errors.Is(p.mintErr, cloudauth.ErrNoFirstClassPath):
		return []diagnosis{{false, fmt.Sprintf(
...
func diagnoseToken(p preflight) []diagnosis {
	...
	if p.target.Cloud == cloudauth.Azure && tok.Audience != "" && p.target.Audience != "" &&
		tok.Audience != p.target.Audience &&
...
func bridgeGuidance(source, target cloudauth.Cloud) string {
	return fmt.Sprintf(
...
func exchangeAdvisory(target cloudauth.Target) string {
	switch target.Cloud {
	case cloudauth.AWS:
		return fmt.Sprintf(
			...
			target.Role, cloudauth.ErrTrustMissing)
	case cloudauth.GCP:
		return fmt.Sprintf(
			...
			target.WorkloadIdentityPool, cloudauth.ErrTrustMissing)
	case cloudauth.Azure:
		return fmt.Sprintf(
			...
			target.Tenant, target.ClientID, cloudauth.ErrTrustMissing)
	default:
		return fmt.Sprintf("unsupported target cloud %q", target.Cloud)
	}
}
