...
func TestCredentialEnvAWS(t *testing.T) {
	c := &cloudauth.Credentials{Cloud: cloudauth.AWS, AccessKeyID: "AKIA", SecretAccessKey: "sk", SessionToken: "st"}
	got := credentialEnv(c)
	want := []string{
...
func TestCredentialEnvBearer(t *testing.T) {
	c := &cloudauth.Credentials{Cloud: cloudauth.GCP, AccessToken: "tok"}
	got := credentialEnv(c)
