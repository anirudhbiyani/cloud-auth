...
func scaffoldTrust(in scaffoldInput) (string, error) {
	...
	switch in.target.Cloud {
	case cloudauth.AWS:
		return scaffoldAWS(in.target, issuer, subject), nil
	case cloudauth.GCP:
		return scaffoldGCP(in.target, issuer, subject), nil
	case cloudauth.Azure:
		return scaffoldAzure(in.target, issuer, subject), nil
	default:
		return "", fmt.Errorf("init: unsupported target cloud %q", in.target.Cloud)
	}
}

func scaffoldAWS(t cloudauth.Target, issuer, subject string) string {
	aud := t.Audience
	if aud == "" {
...
`,
		issuer, aud, t.Role, oidcHost(issuer), subject)
}

func scaffoldGCP(t cloudauth.Target, issuer, subject string) string {
...
func scaffoldAzure(t cloudauth.Target, issuer, subject string) string {
	aud := t.Audience
	if aud == "" {
